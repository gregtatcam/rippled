...
{
    auto& jv = create();
    if (std::holds_alternative<Issue>(book.issue))
    {
        jv["currency"] = to_string(std::get<Issue>(book.issue).currency);
        jv["issuer"] = toBase58(std::get<Issue>(book.issue).account);
    }
    else
    {
        jv["mpt_issuance_id"] =
            to_string(std::get<MPTIssue>(book.issue).getMptID());
    }
}
