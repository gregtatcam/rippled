...
private:
    ...
    testLimitQuality()
    {
        testcase("Limit Quality");
        using namespace jtx;
        Env env(*this);
        Account const gw1("gw1");
        auto const ETH = gw1["ETH"];

        fund(env, gw, {alice, carol, bob}, XRP(2'000), {USD(2'000)});
        env.fund(XRP(2'000), gw1);
        fund(env, gw1, {alice, carol, bob}, {ETH(2'000)}, Fund::IOUOnly);

        env(rate(gw, 1.5));
        env(rate(gw1, 1.9));
        env.close();

        env(offer(bob, ETH(400), USD(250)), txflags(tfPassive));
        env.close();
        AMM amm(env, carol, USD(1'000), ETH(1'000));
