...
private:
    ...
    testInvalidInstance()
    {
        ...
        // Bad MPT
        {
            ...
            AMM ammAlice(
                env, alice, XRP(10'000), MPT(noMPT())(100), ter(temBAD_MPT));
        ...
        testAMM(
            ...
            {.pool = {{AMMMPT(20'000), AMMMPT(10'000)}}});
    ...
    testInvalidDeposit(FeatureBitset features)
    {
        ...
        testAMM(
            ...
            TestAMMArg{.pool = {{XRP(10'000), AMMMPT(10'000)}}});
        ...
        // Individually lock MPT or freeze IOU (AMM) with IOU/MPT AMM
        {
            ...
            {
                // if featureAMMClawback is not enabled, the error will be
                // returned at a different place, asset2 MPT is locked, will
            ...
            BTC.set({.holderID = ammAlice.ammAccount(), .flags = tfMPTLock});
            ...
            // Unlock AMM MPT
            BTC.set({.holderID = ammAlice.ammAccount(), .flags = tfMPTUnlock});
            // can deposit
            ammAlice.deposit(carol, 1'000, std::nullopt, std::nullopt);
            ammAlice.deposit(
                carol, BTC(100), std::nullopt, std::nullopt, std::nullopt);
        ...
        // Individually lock MPT (AMM) account with MPT/MPT AMM
        {
            ...
            USD.set({.holderID = ammAlice.ammAccount(), .flags = tfMPTLock});
            ...
            // Unlock AMM MPT USD
            USD.set({.holderID = ammAlice.ammAccount(), .flags = tfMPTUnlock});
            // Can deposit USD
            ammAlice.deposit(carol, 1'000, std::nullopt, std::nullopt);
            ammAlice.deposit(
                carol, USD(100), std::nullopt, std::nullopt, std::nullopt);
    ...
    testDeposit()
    {
        ...
        testAMM(
            [&](AMM& ammAlice, Env& env) {
                ...
                ammAlice.deposit(carol, USD(200), MPT(ammAlice[1])(100));
                BEAST_EXPECT(ammAlice.expectBalances(
                    USD(10'100),
                    MPT(ammAlice[1])(10'100),
                    IOUAmount{10'100, 0}));
        ...
        // Equal limit deposit.
        // Try to deposit 100MPT0/200MPT1. Is truncated to
        // 100MPT0/100MPT1.
        testAMM(
            [&](AMM& ammAlice, Env& env) {
                ...
                ammAlice.deposit(
                    carol, MPT(ammAlice[0])(100), MPT(ammAlice[1])(100));
                BEAST_EXPECT(ammAlice.expectBalances(
                    MPT(ammAlice[0])(10'100),
                    MPT(ammAlice[1])(10'100),
                    IOUAmount{10'100, 0}));

                env.require(balance(carol, carolMPT0 - MPT(ammAlice[0])(100)));
                env.require(balance(carol, carolMPT1 - MPT(ammAlice[1])(100)));
            },
            {.pool = {{AMMMPT(10'000), AMMMPT(10'000)}}});
        ...
        // Single deposit: 1000 MPT0 into MPT0/MPT1
        testAMM(
            [&](AMM& ammAlice, Env& env) {
                auto carolMPT0 = env.balance(carol, MPT(ammAlice[0]));
                auto carolMPT1 = env.balance(carol, MPT(ammAlice[1]));
        ...
        // Single deposit: 1000 MPT into MPT/USD
        testAMM(
            [&](AMM& ammAlice, Env& env) {
        ...
        // Single deposit: 100 tokens worth of MPT1 into MPT0/MPT1
        testAMM(
            [&](AMM& ammAlice, Env& env) {
                auto carolMPT0 = env.balance(carol, MPT(ammAlice[0]));
                auto carolMPT1 = env.balance(carol, MPT(ammAlice[1]));

                ammAlice.deposit(carol, 100, MPT(ammAlice[1])(205));
                BEAST_EXPECT(ammAlice.expectBalances(
                    MPT(ammAlice[0])(10'000),
                    MPT(ammAlice[1])(10'201),
                    IOUAmount{10'100, 0}));

                env.require(balance(carol, carolMPT0));
                env.require(balance(carol, carolMPT1 - MPT(ammAlice[1])(201)));
            },
            {.pool = {{AMMMPT(10'000), AMMMPT(10'000)}}});

        // Single deposit: 1000 tokens worth of MPT into MPT/USD
        testAMM(
            [&](AMM& ammAlice, Env& env) {
                auto carolUSD = env.balance(carol, USD);
                auto carolMPT = env.balance(carol, MPT(ammAlice[1]));
        ...
        // MPT Issuer create/deposit
        {
            ...
            MPT BTC = MPTTester(
                {.env = env, .issuer = gw, .holders = {}, .assetScale = 6});
        ...
        testAMM(
            ...
            TestAMMArg{
                .pool = {{AMMMPT(10'000), AMMMPT(10'000)}}, .assetScale = 6});
        ...
        testAMM(
            [&](AMM& ammAlice, Env& env) {
                // Equal deposit by tokens MPT/IOU
                ammAlice.deposit(
                    carol,
                    1'000,
                    USD(1'000),
                    ...
                    std::nullopt);
                BEAST_EXPECT(ammAlice.expectBalances(
                    USD(11'000), MPT(ammAlice[1])(11'000), IOUAmount{11'000}));
            },
            {{USD(10'000), AMMMPT(10'000)}});
        ...
        testAMM(
            [&](AMM& ammAlice, Env& env) {
                // Equal deposit by asset USD/MPT
                ammAlice.deposit(
                    carol,
                    1'000,
                    USD(1'000),
        ...
        testAMM(
            [&](AMM& ammAlice, Env& env) {
                // Single deposit by asset MPT/XRP
                ammAlice.deposit(
                    carol,
                    488'088,
                    XRP(1'000),
        ...
        testAMM(
            [&](AMM& ammAlice, Env& env) {
                // Single deposit by asset MPT/USD
                ammAlice.deposit(
                    carol,
                    488,
                    USD(1'000),
    ...
    testInvalidWithdraw()
    {
        ...
        // Globally locked MPT
        {
            ...
            // returned from AMMWithdraw.cpp:preclaim(): isMPTTxAllowed()
            ammAlice.withdraw(
                {.account = alice,
                 .asset1Out = XRP(100),
        ...
        // Individually frozen (AMM) account with MPT/MPT AMM
        {
            ...
            // Error returned at a different place: from isMPTTxAllowed()
            ammAlice.withdraw(
                alice,
                USD(100),
                std::nullopt,
        ...
        // Individually lock MPT or freeze IOU (AMM)
        {
            ...
            // asset2 MPT is checked. todo: this should not block withdraw
            ammAlice.withdraw(
                alice,
                USD(100),
                std::nullopt,
    ...
    testWithdraw()
    {
        ...
        // Equal withdrawal with a limit. XRP100/MPT100.
        testAMM(
            [&](AMM& ammAlice, Env&) {
        ...
        // Single deposit 1000MPT, withdraw all tokens in XRP
        testAMM(
            [&](AMM& ammAlice, Env&) {
                ammAlice.deposit(carol, MPT(ammAlice[1])(1'000));
                ...
                BEAST_EXPECT(ammAlice.expectBalances(
                    ...
                    STAmount{
                        MPT(ammAlice[1]), UINT64_C(10'999'99999999999), -11},
                    IOUAmount{10'000'000, 0}));
            },
            {{XRP(10'000), AMMMPT(10'000)}});
        ...
        // Equal deposit 10%, withdraw all tokens in MPT
        testAMM(
            [&](AMM& ammAlice, Env&) {
                ammAlice.deposit(carol, 1'000'000);
                ammAlice.withdrawAll(carol, MPT(ammAlice[1])(0));
        ...
        // MPT to MPT + transfer fee
        {
            ...
            MPTTester BTC(
                {.env = env,
                 ...
                 .holders = {alice, carol},
                 .transferFee = 25'000});
            env(pay(gw, alice, BTC(2'000)));
            MPTTester ETH(
                {.env = env,
                 .issuer = gw,
                 .holders = {alice, carol},
                 .transferFee = 25'000});
            env(pay(gw, alice, ETH(5)));
            env.close();
            // no transfer fee on create
            AMM ammAlice(env, alice, BTC(2'000), ETH(5));
            BEAST_EXPECT(
                ammAlice.expectBalances(BTC(2'000), ETH(5), IOUAmount{100, 0}));
            env.require(balance(alice, BTC(0)));
            env.require(balance(alice, ETH(0)));
            env(pay(gw, carol, BTC(2'000)));
            env(pay(gw, carol, ETH(5)));
            // no transfer fee on deposit
            ammAlice.deposit(carol, 100);
            BEAST_EXPECT(
                ammAlice.expectBalances(BTC(4000), ETH(10), IOUAmount{200, 0}));
            env.require(balance(carol, BTC(0)));
            env.require(balance(carol, ETH(0)));
            // no transfer fee on withdraw
            ammAlice.withdraw(carol, 100);
            BEAST_EXPECT(
                ammAlice.expectBalances(BTC(2'000), ETH(5), IOUAmount{100, 0}));
            BEAST_EXPECT(ammAlice.expectLPTokens(carol, IOUAmount{0, 0}));
            env.require(balance(carol, BTC(2'000)));
            env.require(balance(carol, ETH(5)));
        }
    ...
    testInvalidAMMPayment()
    {
        ...
        //     AMM ammAlice(env, alice, XRP(10'000), BTC(10'000));
        //     BTC.set({.flags = tfMPTLock, .holderID = ammAlice.ammAccount()});

        // //todo: get tesSUCCESS instead of tecLOCKED
        //     env(pay(alice, carol, XRP(1)),
        //         path(~XRP),
        //         txflags(tfPartialPayment | tfNoRippleDirect),
        //         sendmax(BTC(10)),
        ...
        /*
                ...
                testAMM(
                    [&](AMM& ammAlice, Env& env) {
                        ...
                        env.close();
                        BEAST_EXPECT(ammAlice.expectBalances(
                            XRP(10'100), MPT(ammAlice[1])(10'000),
           ammAlice.tokens()));
                        // Initial balance + 100
                        env.require(balance(carol, carolMPT +
           MPT(ammAlice[1])(100)));
           ...
           MPT(ammAlice[1])(100)));
                ...
                testAMM(
                    [&](AMM& ammAlice, Env& env) {
                        ...
                        env.close();
                        // Pays 10MPT for 10XRP. A larger payment of
           ~99.11MPT/100XRP
                        // would have been sent has it not been for
           limitQuality. env(pay(bob, carol, MPT(ammAlice[1])(100)),
                            path(~MPT(ammAlice[1])),
                            sendmax(XRP(100)),
                            txflags(
                                tfNoRippleDirect | tfPartialPayment |
           tfLimitQuality)); env.close(); BEAST_EXPECT(ammAlice.expectBalances(
                            XRP(10'010), MPT(ammAlice[1])(10'000),
           ammAlice.tokens()));
           ...
           - 10(tx
                        ...
                        env(pay(bob, carol, MPT(ammAlice[1])(100)),
                            ...
                            txflags(
                                tfNoRippleDirect | tfPartialPayment |
           tfLimitQuality), ter(tecPATH_DRY)); env.close();
                    },
                ...
                // Payment with limitQuality and transfer fee set.
                {
                    ...
                    MPTTester BTC(
                        {.env = env,
                         ...
                         .flags = MPTDEXFlags});
                    auto ammAlice =
                        AMM(env, alice, XRP(10'000),
           BTC(10'010'000'000'000'000)); env.fund(jtx::XRP(30'000), bob);
                    env.close();
                    auto carolMPT = env.balance(carol, MPT(BTC));
                    // Pays 10'000'000'000'000MPT for 10XRP. A larger payment of
                    // ~99'110'000'000'000MPT/100XRP would have been sent has it
           not
           ...
           tfLimitQuality)); env.close(); BEAST_EXPECT(ammAlice.expectBalances(
                        XRP(10'010), BTC(10'000'000'000'000'000),
           ammAlice.tokens()));
                    // 10'000'000'000'000MPT - 10% transfer fee
                    env.require(balance(carol, carolMPT +
           BTC(9'090'909'090'909))); BEAST_EXPECT(expectLedgerEntryRoot( env,
           bob, XRP(30'000) - XRP(10) - txfee(env, 1)));
                }
        ...
        /*
           ...
           non-default path.
                ...
                // path ~49XRP/49e14BTC. The rest is taken from the offer.
                {
                    ...
                    auto ammAlice =
                        AMM(env, alice, XRP(10'000),
           BTC(1'000'000'000'000'000'000)); env.fund(XRP(1'000), bob);
                    env.close();
                    ...
                    BEAST_EXPECT(expectOffers(
                        ...
                        alice,
                        2,
                        {{Amounts{XRPAmount(50'487'378),
           ETH(49'98750312422000)}, Amounts{ETH(49'98750312422000),
           BTC(49'98750312422000)}}}));
                    // Initial (30,000 + 100)e14
                    env.require(balance(carol, BTC(30'100'00000000000000)));
                    // Initial 1,000 - 50238637(AMM pool) - 50512622(offer) -
           10(tx
                    // fee)
                    BEAST_EXPECT(expectLedgerEntryRoot(
                        env,
                        bob,
                        XRP(1'000) - XRPAmount{50'238'637} -
           XRPAmount{50'512'622} - txfee(env, 1)));
                }

                // Default path with AMM and Order Book offer. AMM is consumed
           first,
                // remaining amount is consumed by the offer.
                {
                    ...
                    MPTTester BTC(
                        {.env = env,
                         ...
                         .pay = 30'000'000'000'000'000,
                         .flags = MPTDEXFlags});
                    AMM ammAlice(env, alice, XRP(10'000),
           BTC(10'100'000'000'000'000)); env(offer(bob, XRP(100),
           MPT(ammAlice[1])(100'000'000'000'000)), txflags(tfPassive));
                    env.close();
                    env(pay(alice, carol,
           MPT(ammAlice[1])(200'000'000'000'000)), sendmax(XRP(200)),
                        txflags(tfPartialPayment));
                    env.close();
           ...
           MPT(ammAlice[1])(30'199'999999999990)));

                    // Initial 30,000 - 10000(AMM pool LP) - 100(AMM offer) -
                    // - 100(offer) - 10(tx fee) - 10(tx fee of MPTTester init
           as
                    // holder) - one reserve
                    BEAST_EXPECT(expectLedgerEntryRoot(
                        env,
           ...
           ammAlice.tokens()));
                    ...
                    // Initial 30,000 - 100(offer) - 10(tx fee) - 1(tx fee for
           MPTTester
                    // holder)
                    BEAST_EXPECT(expectLedgerEntryRoot(
                ...
                // Single path AMM offer
                {
                    ...
                    env(offer(
                        carol, ETH(100'000'000'000'000),
           BTC(100'000'000'000'000))); env.close();
                    // No transfer fee
                    BEAST_EXPECT(ammAlice.expectBalances(
                        BTC(1'100'000'000'000'000),
                        ETH(1'000'000'000'000'000),
                        ammAlice.tokens()));
                    // Initial 30,000'000'000'000'000 -
           100'000'000'000'000(offer) - 25%
                    // transfer fee
                    env.require(balance(carol, BTC(29'875'000'000'000'000)));
                    // Initial 30,000'000'000'000'000 +
           100'000'000'000'000(offer) env.require(balance(carol,
           ETH(30'100'000'000'000'000))); BEAST_EXPECT(expectOffers(env, carol,
           0));
                }

                // Single-path AMM offer
                {
                    // [TODO] amout diff due to rounding
                    Env env(*this);
                    ...
                    BEAST_EXPECT(amm.expectBalances(
                        XRPAmount(909'090'909), BTC(550'000000055000),
           amm.tokens()));
                    ...
                    BEAST_EXPECT(expectOffers(
                        ...
                        carol,
                        1,
                        {{Amounts{XRPAmount{9'090'909},
           BTC(4'999999950000)}}}));
                    // Carol pays 0.1% fee on 50'000000055000BTC =
           50'000000055BTC env.require(balance(carol,
           BTC(29'949'949'999'944'945)));
                }

                {
                    ...
                    MPTTester BTC(
                        {.env = env,
                         ...
                         .pay = 3'000'000'000'000'000'000,
                         .flags = MPTDEXFlags});
                    AMM amm(env, alice, XRP(1'000),
           BTC(50'000'000'000'000'000)); env(offer(carol, XRP(10),
           BTC(5'500'000'000'000'000))); env.close();
                ...
                // Multi-path AMM offer
                {
                    ...
                    env.close();
                    env(offer(
                        carol, ETH(1'000'000'000'000'000),
           BTC(1'000'000'000'000'000))); env.close();
                    ...
                    BEAST_EXPECT(expectOffers(
                        ...
                        carol,
                        1,
                        {Amounts{ETH(27'0658372213403),
           BTC(27'0658372213403)}})); BEAST_EXPECT(expectOffers(env, bob, 0));
                ...
                // Multiple paths/steps
                {
                    ...
                    MPTTester EUR(
                        {.env = env,
                         ...
                         .pay = 500'000'000'000'000'000,
                         .flags = MPTDEXFlags});
                    AMM xrp_eur(env, alice, XRP(10'100),
           EUR(100'000'000'000'000'000)); AMM eur_btc( env, alice,
                    ...
                    AMM xrp_usd(env, alice, XRP(10'150),
           USD(102'000'000'000'000'000)); AMM xrp_eth(env, alice, XRP(10'000),
           ETH(101'000'000'000'000'000)); AMM eth_eur( env, alice,
           ...
           xrp_eur.tokens())); BEAST_EXPECT(eur_btc.expectBalances(
                        EUR(100'000'000'000'000'000),
                        BTC(102'000'000'000'000'000),
                        eur_btc.tokens()));
                ...
                // Dependent AMM
                {
                    ...
                    MPTTester EUR(
                        {.env = env,
                         ...
                         .flags = MPTDEXFlags});
                    AMM xrp_eur(env, alice, XRP(10'100),
           EUR(10'000'000'000'000'000)); AMM eur_btc( env, alice,
                    ...
                    AMM xrp_eth(env, alice, XRP(10'000),
           ETH(10'100'000'000'000'000)); AMM eth_eur( env, alice,
                ...
                // AMM offers limit
                // Consuming 30 CLOB offers, results in hitting 30 AMM offers
           limit.
                {
                    Env env(*this);
                    env.fund(XRP(30'000), gw, alice, carol);
           ...
           BTC(10'000'000'000'000'000));
                    ...
                    BEAST_EXPECT(ammAlice.expectBalances(
                        XRP(10'030), BTC(9'970'089730807827),
           ammAlice.tokens()));

                    env.require(balance(carol, BTC(30'029'910269192173)));
                    BEAST_EXPECT(expectOffers(
                        env, alice, 1, {{{ETH(140'000'000'000'000),
           XRP(100)}}}));
                }
           ...
           BTC(10'000'000'000'000'000));
                ...
                // quality and is consumed first.
                {
                    ...
                    AMM ammAlice(env, alice, XRP(10'000),
           BTC(10'100'000'000'000'000)); env(offer(carol,
           BTC(100'000'000'000'000), XRP(100)));
    ...
    testTradingFee(FeatureBitset features)
    {
        ...
        testAMM(
            [&](AMM& ammAlice, Env& env) {
                ...
                BEAST_EXPECT(ammAlice.expectLPTokens(carol, IOUAmount{0}));
                env.require(balance(carol, MPT(ammAlice[1])(300'000)));
                // Set fee to 1%
                ...
                env.require(balance(carol, MPT(ammAlice[1])(297'000)));
                ...
                env.require(balance(carol, MPT(ammAlice[1])(299'995)));
            },
        ...
        testAMM(
            [&](AMM& ammAlice, Env& env) {
                auto const balance = env.balance(carol, MPT(ammAlice[0]));
                auto const tokensFee = ammAlice.deposit(
                    ...
                    MPT(ammAlice[0])(1));
                auto const deposit =
                    balance - env.balance(carol, MPT(ammAlice[0]));
                ammAlice.withdrawAll(carol, MPT(ammAlice[0])(0));
                ammAlice.vote(alice, 0);
                BEAST_EXPECT(ammAlice.expectTradingFee(0));
                auto const tokensNoFee = ammAlice.deposit(carol, deposit);
                BEAST_EXPECT(tokensFee == IOUAmount(1303280911962123, -12));
                BEAST_EXPECT(tokensNoFee == IOUAmount(1303185121322776, -12));
            },
            {{AMMMPT(1'000), USD(10'000)}},
            1'000,
            std::nullopt,
            {features},
            1);
        ...
        testAMM(
            [&](AMM& ammAlice, Env& env) {
                ...
                BEAST_EXPECT(ammAlice.expectTradingFee(0));
                auto const tokensNoFee = ammAlice.deposit(carol, deposit);
                BEAST_EXPECT(tokensFee == IOUAmount(99'004'92586205000, -11));
                BEAST_EXPECT(tokensNoFee == IOUAmount(99'485'13935160000, -11));
            },
            {{XRP(10'000),
              AMMMPT(
                  10'000)}},  // todo: test framework, switch MPT and XRP order
            1'000,
            std::nullopt,
            {features},
            6);
        ...
        testAMM(
            [&](AMM& ammAlice, Env& env) {
                ...
                env.require(balance(carol, MPT(ammAlice[1])(300443)));
                BEAST_EXPECT(tokensNoFee == IOUAmount(746'327'4649664900, -10));
                BEAST_EXPECT(tokensFee == IOUAmount(750'588'2352941100, -10));
            },
            {{XRP(10'000), AMMMPT(10'000)}},
            1'000,
            std::nullopt,
        ...
        testAMM(
            [&](AMM& ammAlice, Env& env) {
                ...
                env.close();
                env.require(balance(carol, BTC(300'000'000'000'000010)));
                env.require(balance(carol, XRP(29989.99998)));

                // Change pool composition back
                env(offer(carol, XRP(10), BTC(10)));
                env.close();
                env.require(balance(carol, BTC(300'000'000'000'000000)));
                ...
                env.require(balance(carol, BTC(300'000'000'000'000009)));
                env.require(balance(carol, XRP(29991.004453)));
                BEAST_EXPECT(
                    expectOffers(env, carol, 1, {{Amounts{BTC(1), XRP(1)}}}));
            },
            {.pool = {{XRP(1000), AMMMPT(1010)}}, .assetScale = 6});

        // Offer crossing, 0.5% fee MPT/IOU
        testAMM(
            [&](AMM& ammAlice, Env& env) {
                auto const BTC = MPT(ammAlice[1]);
                env(offer(carol, BTC(10000000), USD(10)));
                env.close();
                env.require(balance(carol, BTC(300'000'000'010'000000)));
                env.require(balance(carol, USD(29'990)));

                // Change pool composition back
                env(offer(carol, USD(10), BTC(10000000)));
                env.close();
                env.require(balance(carol, BTC(300'000'000'000'000000)));
                ...
                env.require(balance(carol, BTC(300'000'000'004'974874)));
                env.require(balance(
            ...
            {.pool = {{USD(1000), AMMMPT(1010000000)}}, .assetScale = 6});
    ...
    testFixAMMOfferBlockedByLOB(FeatureBitset features)
    {
        ...
        // USD/MPT crosses AMM despite of low quality LOB
        {
            ...
            BEAST_EXPECT(
                amm.expectBalances(BTC(200'001), USD(99'999.51), amm.tokens()));
            BEAST_EXPECT(
                expectOffers(env, alice, 1, {{Amounts{BTC(1), USD(0.01)}}}));
        ...
        // XRP/MPT crosses AMM despite of low quality LOB
        {
            ...
            BEAST_EXPECT(
                amm.expectBalances(BTC(200'001), XRP(99'999.51), amm.tokens()));
            BEAST_EXPECT(
                expectOffers(env, alice, 1, {{Amounts{BTC(1), XRP(0.01)}}}));
            // Carol's offer crosses AMM
            BEAST_EXPECT(expectOffers(env, carol, 0));
        }
    }
    ...
    testLPTokenBalance(FeatureBitset features)
    {
        ...
        {
            amm.withdrawAll(alice, std::nullopt, ter(tecAMM_BALANCE));
    ...
    run() override
    {
        ...
        // testClawback();
        // testClawbackFromAMMAccount();
        // testInvalidAMMPayment();  // regression

        testBasicPaymentEngine();  // regression

        // testAMMTokens();
        // testAmendment();
        // testAMMAndCLOB(all);
        // testTradingFee(all); // regression
        // testAdjustedTokens(all);
        // testAMMID();
        // testSelection(all); // regression
        // testMalformed();
