...
class AMMClawbackMPT_test : public jtx::AMMTest
{
    ...
    testInvalidRequest()
    {
        ...
        // Test if the issuer did not set asfAllowTrustLineClawback,
        // and the issuer tries to claw two assets at a time, asset2 is IOU.
        // AMMClawback transaction is prohibited.
        {
            Env env(*this);
            Account gw{"gateway"};
            Account alice{"alice"};
            ...
            // claw USD is not allowed without asfAllowTrustLineClawback, USD is
            // IOU.
            env(amm::ammClawback(gw, alice, USD, BTC, std::nullopt),
                ter(tecNO_PERMISSION));

            // but only claw BTC is permitted, BTC is MPT
            env(amm::ammClawback(gw, alice, BTC, USD, std::nullopt));
            BEAST_EXPECT(!amm.ammExists());
        }
    }
    ...
    testAMMClawbackAmount()
    {
        ...
        // AMMClawback from MPT/IOU issued by different issuers
        {
            ...
            BEAST_EXPECT(amm.expectBalances(
                ...
                IOUAmount{353553'3905932737, -10}));
        ...
        // AMMClawback from MPT/XRP pool
        {
            ...
            env.require(balance(alice, aliceXRP));
            env.require(balance(alice, aliceBTC));
            env.require(balance(bob, bobXRP + XRP(2000) + drops(1)));
            env.require(balance(bob, bobBTC));
        }
    ...
    testAMMClawbackAmountSameIssuer()
    {
        ...
        // AMMClawback from MPT/IOU issued by the same issuer
        {
            ...
            BEAST_EXPECT(amm.expectBalances(
                BTC(1'000'000001),
                STAmount{USD, UINT64_C(2000'000000399999), -12},
            ...
            BEAST_EXPECT(amm.expectBalances(
                BTC(250'000001),
                STAmount{USD, UINT64_C(500'000000399999), -12},
            ...
            env.require(balance(alice, aliceUSD));
            env.require(balance(alice, aliceBTC));
            // USD is also clawed back from bob because of tfClawTwoAssets, so
        ...
        // AMMClawback from MPT/MPT issued by the same issuer
        {
            ...
            // gw clawback 3'000'000000 ETH from bob
            env(amm::ammClawback(gw, bob, ETH, BTC, ETH(3'000'000000)));
            env.close();
            BEAST_EXPECT(
                amm.expectBalances(BTC(1), ETH(1), IOUAmount{816496, -6}));
    ...
    testAMMClawbackIssuesEachOther()
    {
        ...
        // AMMClawback from MPT/IOU issued by each other
        {
            ...
            env.close();
            BEAST_EXPECT(amm.expectBalances(
                USD(5000), BTC(10000), IOUAmount{7071067811865474, -12}));
            BEAST_EXPECT(
                amm.expectLPTokens(gw, IOUAmount{1414213562373095, -12}));
            BEAST_EXPECT(
            ...
            env.close();
            BEAST_EXPECT(amm.expectBalances(
                USD(4500), BTC(9000), IOUAmount{6363'961030678927, -12}));
            ...
            env.close();
            BEAST_EXPECT(amm.expectBalances(
                USD(2500), BTC(5000), IOUAmount{3535533905932737, -12}));
        ...
        // AMMClawback from MPT/MPT issued by each other
        {
            ...
            env.close();
            BEAST_EXPECT(amm.expectBalances(
                BTC(69'000), ETH(345'001), IOUAmount{154'288'6904474855, -10}));
            env.require(balance(alice, aliceBTC));
            env.require(balance(alice, aliceETH));
    ...
    testAssetFrozenOrLocked()
    {
        ...
        // test AMMClawback when MPT globally locked or IOU globally frozen
        {
            ...
            BEAST_EXPECT(ammAlice.expectBalances(
                STAmount{USD, UINT64_C(7000'000000000001), -12},
                BTC(7'000),
                IOUAmount(7'000)));
            env.require(balance(alice, aliceBTC));
        ...
        // frozen
        {
            ...
            BEAST_EXPECT(ammAlice.expectBalances(
                USD(4'000), BTC(4'000), IOUAmount{3999'999999999999, -12}));
            env.require(balance(alice, aliceBTC));
            ...
            env(amm::ammClawback(
                gw, alice, USD, static_cast<MPT>(BTC), USD(1'000)));
            BEAST_EXPECT(ammAlice.expectBalances(
                USD(3'000), BTC(3'001), IOUAmount(3'000)));
            env.require(balance(alice, aliceBTC + BTC(999)));
            env.require(balance(alice, aliceUSD));
        }
    }
    ...
    testLastHolderLPTokenBalance(FeatureBitset features)
    {
        ...
        if (!features[fixAMMClawback])
        {
            ...
            BEAST_EXPECT(amm.expectBalances(
                BTC(2),
                STAmount(USD, UINT64_C(6666666666671334), -16),
                IOUAmount{9428090415826667, -16}));
        }
        else
        {
            env(amm::ammClawback(gw, alice, BTC, USD, BTC(1)));
    ...
    run() override
    {
        ...
        testLastHolderLPTokenBalance(all - fixAMMClawback);
    }
};
