...
namespace ripple {
namespace test {
class AMMClawbackMPT_test : public jtx::AMMTest
{
    void
    testInvalidRequest()
    {
        ...
        // transaction is prohibited.
        {
            ...
            AMM amm(env, gw, BTC(100), XRP(100));
            env.close();
    ...
    testAMMClawbackAmount()
    {
        ...
        // AMMClawback from MPT/XRP pool
        {
            ...
            BEAST_EXPECT(amm.expectBalances(
                BTC(2'999'999500),
                STAmount{XRP, UINT64_C(5'999'999000)},
                IOUAmount{4'242'639'980'012504, -6}));
            env.require(balance(alice, aliceXRP + drops(1000)));
            env.require(balance(alice, aliceBTC));
            ...
            BEAST_EXPECT(amm.expectBalances(
                BTC(1'999'999500),
                STAmount{XRP, UINT64_C(3'999'999000)},
                IOUAmount{2'828'426'417'639409, -6}));
            env.require(balance(alice, aliceXRP));
            env.require(balance(alice, aliceBTC));
            env.require(balance(bob, bobXRP + XRP(2000)));
            env.require(balance(bob, bobBTC));
            BEAST_EXPECT(
                amm.expectLPTokens(alice, IOUAmount{1'414'212'855'266314, -6}));
            BEAST_EXPECT(
                amm.expectLPTokens(bob, IOUAmount{1'414'213'562'373095, -6}));
        ...
        // AMMClawback from MPT/MPT pool, different issuers
        {
            ...
            BEAST_EXPECT(amm.expectBalances(
                BTC(5'000'000000),
                ETH(7'500'000000),
                IOUAmount{6'123'724'356'957945, -6}));
            env.require(balance(alice, aliceBTC));
            env.require(balance(alice, aliceETH + ETH(1'500'000000)));
            env.require(balance(bob, bobBTC));
            env.require(balance(bob, bobETH));
            BEAST_EXPECT(
                amm.expectLPTokens(alice, IOUAmount{1'224'744'871'391589, -6}));
            BEAST_EXPECT(
            ...
            BEAST_EXPECT(amm.expectBalances(
                BTC(3'000'000000),
                ETH(4'500'000000),
                IOUAmount{3'674'234'614'174767, -6}));
            env.require(balance(alice, aliceBTC));
            ...
            BEAST_EXPECT(amm.expectBalances(
                ...
                ETH(3'000'000000),
                IOUAmount{2'449'489'742'783178, -6}));
            env.require(balance(alice, aliceBTC + BTC(1'000'000000)));
            env.require(balance(alice, aliceETH));
            env.require(balance(bob, bobBTC));
            env.require(balance(bob, bobETH));
            aliceBTC = env.balance(alice, BTC);
            ...
            env.require(balance(alice, aliceETH));
            env.require(balance(bob, bobBTC));
            env.require(balance(bob, bobETH + ETH(3'000'000000)));
        }
    }
    ...
    testAMMClawbackAll()
    {
        ...
        /*
                ...
                // AMMClawback all from MPT/XRP pool
                {
                    ...
                    env.close();
                    BEAST_EXPECT(amm.expectBalances(
                        BTC(5'000), XRP(10'000), IOUAmount{7'071'067'811865475,
           -9}));

                    amm.deposit(bob, BTC(10'000), XRP(20'000));
                    BEAST_EXPECT(amm.expectBalances(
                        BTC(15'000), XRP(30'000), IOUAmount{21'213'203'43559642,
           -8}));
                    ...
                    env.close();
                    BEAST_EXPECT(amm.expectBalances(
                        BTC(10'000), XRP(20'000), IOUAmount{14'142'135'62373094,
           -8})); env.require(balance(alice, aliceBTC));
    ...
    void
    testAMMClawbackAmountSameIssuer()
    {
        testcase(
        ...
        // AMMClawback from MPT/IOU issued by the same issuer
        {
            ...
            BEAST_EXPECT(amm.expectBalances(
                BTC(1250'000000),
                STAmount{USD, UINT64_C(2'499'999999999999), -12},
                IOUAmount{1'767'766'952966368, -9}));
            env.require(balance(alice, aliceUSD));
            env.require(balance(alice, aliceBTC + BTC(250'000000)));
            env.require(balance(bob, bobUSD));
            env.require(balance(bob, bobBTC));
            aliceBTC = env.balance(alice, BTC);
            ...
            BEAST_EXPECT(amm.expectBalances(
                BTC(1'000'000000),
                STAmount{USD, UINT64_C(1'999'999999999999), -12},
                IOUAmount{1'414'213'562373094, -9}));
            env.require(balance(alice, aliceUSD));
            env.require(balance(alice, aliceBTC));
            env.require(balance(bob, bobUSD));
            env.require(balance(bob, bobBTC));
            BEAST_EXPECT(
                amm.expectLPTokens(alice, IOUAmount{1'060'660'171779821, -9}));
            BEAST_EXPECT(
                amm.expectLPTokens(bob, IOUAmount{353'553'3905932734, -10}));
            ...
            BEAST_EXPECT(amm.expectBalances(
                BTC(250'000000),
                STAmount{USD, UINT64_C(499'999999999999), -12},
                IOUAmount{353'553'390593273, -9}));
            env.require(balance(alice, aliceUSD));
            env.require(balance(alice, aliceBTC + BTC(750'000000)));
            env.require(balance(bob, bobUSD));
            env.require(balance(bob, bobBTC));
            BEAST_EXPECT(amm.expectLPTokens(alice, IOUAmount(0)));
            BEAST_EXPECT(
    ...
    void
    testAMMClawbackIssuesEachOther()
    {
        ...
        // AMMClawback from MPT/IOU issued by each other
        {
            ...
            env.close();
            BEAST_EXPECT(amm.expectBalances(
                USD(1000), BTC(2000), IOUAmount{1414213562373095, -12}));

            amm.deposit(gw2, USD(2000), BTC(4000));
            BEAST_EXPECT(amm.expectBalances(
                USD(3000), BTC(6000), IOUAmount{4242640687119285, -12}));

            amm.deposit(alice, USD(3000), BTC(6000));
            BEAST_EXPECT(amm.expectBalances(
                USD(6000), BTC(12000), IOUAmount{8485281374238570, -12}));

            BEAST_EXPECT(
                amm.expectLPTokens(gw, IOUAmount{1414213562373095, -12}));
            BEAST_EXPECT(
                amm.expectLPTokens(gw2, IOUAmount{2828427124746190, -12}));
            BEAST_EXPECT(
            ...
            env.close();
            BEAST_EXPECT(amm.expectBalances(
                USD(5000), BTC(10000), IOUAmount{7071067811865475, -12}));
            BEAST_EXPECT(
                amm.expectLPTokens(gw, IOUAmount{1414213562373095, -12}));
            BEAST_EXPECT(
            ...
            env.close();
            BEAST_EXPECT(amm.expectBalances(
                USD(4500), BTC(9000), IOUAmount{6363961030678928, -12}));
            ...
            env.close();
            BEAST_EXPECT(amm.expectBalances(
                USD(2500), BTC(5000), IOUAmount{3535533905932738, -12}));
        ...
        // AMMClawback from MPT/MPT issued by each other
        {
            ...
            env.close();
            BEAST_EXPECT(amm.expectBalances(
                BTC(10'000), ETH(50'000), IOUAmount{22'360'6797749979, -10}));

            amm.deposit(gw2, BTC(20'000), ETH(100'000));
            BEAST_EXPECT(amm.expectBalances(
                BTC(30'000), ETH(150'000), IOUAmount{67'082'0393249937, -10}));
            ...
            env.close();
            BEAST_EXPECT(amm.expectBalances(
                BTC(69'000), ETH(345'000), IOUAmount{154'288'6904474855, -10}));
            env.require(balance(alice, aliceBTC));
            env.require(balance(alice, aliceETH));
            ...
            env.close();
            BEAST_EXPECT(amm.expectBalances(
                BTC(59'000), ETH(295'000), IOUAmount{131'928'0106724876, -10}));
            env.require(balance(alice, aliceBTC));
            env.require(balance(alice, aliceETH));
            env.require(balance(gw, gwETH));
            env.require(balance(gw2, gw2BTC));
            ...
            env.close();
            BEAST_EXPECT(amm.expectBalances(
                BTC(19'000), ETH(95'000), IOUAmount{42'485'2915724961, -10}));
            env.require(balance(alice, aliceBTC));
    ...
    testAssetFrozenOrLocked()
    {
        ...
        // test AMMClawback when MPT globally locked or IOU globally frozen
        {
            ...
            auto aliceBTC = env.balance(alice, static_cast<MPT>(BTC));
            auto aliceUSD = env.balance(alice, USD);
            ...
            env(amm::ammClawback(
                gw, alice, static_cast<MPT>(BTC), USD, BTC(1'000)));
            BEAST_EXPECT(ammAlice.expectBalances(
            ...
            env(amm::ammClawback(
                gw, alice, static_cast<MPT>(BTC), USD, BTC(2'000)));
            BEAST_EXPECT(ammAlice.expectBalances(
            ...
            env.close();
            env(amm::ammClawback(
                gw, alice, USD, static_cast<MPT>(BTC), USD(1'000)));
            BEAST_EXPECT(ammAlice.expectBalances(
                STAmount{USD, UINT64_C(6'000'000000000001), -12},
                BTC(6'000),
                IOUAmount{6'000'000000000001, -12}));
            env.require(balance(alice, aliceBTC + BTC(1'000)));
            env.require(balance(alice, aliceUSD));
            aliceBTC = env.balance(alice, static_cast<MPT>(BTC));
            ...
            env.close();
            env(amm::ammClawback(
                    gw, alice, USD, static_cast<MPT>(BTC), USD(2'000)),
                txflags(tfClawTwoAssets));
            BEAST_EXPECT(ammAlice.expectBalances(
                STAmount{USD, UINT64_C(4'000'000000000001), -12},
                BTC(4'000),
        ...
        // frozen
        {
            ...
            env(amm::ammClawback(
                gw, alice, static_cast<MPT>(BTC), USD, BTC(3'000)));
            BEAST_EXPECT(ammAlice.expectBalances(
    ...
    testSingleDepositAndClawback()
    {
        ...
        env(amm::ammClawback(gw, alice, static_cast<MPT>(BTC), USD, BTC(100)));
        BEAST_EXPECT(amm.expectBalances(
        ...
        // gw clawback 30 USD from alice with tfClawTwoAssets, which exceeds her
        // balance
        env(amm::ammClawback(gw, alice, USD, static_cast<MPT>(BTC), USD(30)),
            txflags(tfClawTwoAssets));
        BEAST_EXPECT(amm.expectBalances(
