...
PathRequests::getAssetCache(std::shared_ptr<ReadView const> const& ledger, bool authoritative)
{
        ...
        (lgrSeq > (lineSeq + 8)))                       // we jumped way forward for some reason
    {
        ...
        // references.
        assetCache_ = assetCache = std::make_shared<AssetCache>(ledger, app_.journal("AssetCache"));
    }
    return assetCache;
}
...
PathRequests::doLegacyPathRequest(
    ...
    Json::Value const& request)
{
    auto cache = std::make_shared<AssetCache>(inLedger, app_.journal("AssetCache"));
