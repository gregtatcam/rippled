...
Payment::preflight(PreflightContext const& ctx)
{
    ...
    else if (saDstAmount.native() || saDstAmount.holds<MPTIssue>())
        maxSourceAmount = saDstAmount;
    else
        maxSourceAmount = STAmount(
            Issue{saDstAmount.get<Issue>().currency, account},
    ...
    bool const bXRPDirect = isXRP(maxSourceAmount) && isXRP(saDstAmount);

    if (!isLegalNet(uDstAsset) || !isLegalNet(uSrcAsset))
    ...
    if (account == uDstAccountID &&
        equalCurrencyOrMPTID(uSrcAsset, uDstAsset) && !bPaths)
    {
    ...
    if (deliverMin)
    {
        ...
        // TODO is tfNoRippleDirect valid with MPT?
    }
...
Payment::doApply()
{
    ...
    if (sendMax)
        ...
        maxSourceAmount = STAmount(
            Issue{saDstAmount.get<Issue>().currency, account_},
            saDstAmount.mantissa(),
            saDstAmount.exponent(),
            saDstAmount < beast::zero);
    ...
    if (bRipple)
    {
        ...
        rcInput.isLedgerOpen = ctx_.view().open();

        path::RippleCalc::Output rc;
        {
            PaymentSandbox pv(&ctx_.view());
            JLOG(ctx_.journal.debug()) << "Entering RippleCalc in payment: "
    ...
    auto const sleSrc = ctx_.view().peek(keylet::account(account_));
    if (!sleSrc)
        return tefINTERNAL;
    ...
    if (mPriorBalance < saDstAmount.xrp() + mmm)
    {
        ...
        // a different order.
        JLOG(ctx_.journal.trace()) << "Delay transaction: Insufficient funds: "
        ...
        if (uDstAccountID != account_)
        {
                    ...
                    keylet::depositPreauth(uDstAccountID, account_)))
            {
                ...
                XRPAmount const dstReserve{
                    ctx_.view().fees().accountReserve(0)};
