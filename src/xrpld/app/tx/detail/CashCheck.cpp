...
CashCheck::doApply()
{
    ...
    if (srcId != account_)
    {
        ...
        else
        {
            ...
            auto checkReserve = [&]() -> std::pair<TER, std::shared_ptr<SLE>> {
                    ...
                    mPriorBalance < psb.fees().accountReserve(ownerCount + 1))
                {
                    ...
                    return std::make_pair(tecNO_LINE_INSUF_RESERVE, nullptr);
                }
                return std::make_pair(tesSUCCESS, sleDst);
            };
            ...
            if (flowDeliver.holds<Issue>())
            {
                ...
                if (checkCashMakesTrustLine && !psb.exists(*trustLineKey))
                {
                    ...
                    auto const [err, sleDst] = checkReserve();
                    if (err != tesSUCCESS)
                        return err;
