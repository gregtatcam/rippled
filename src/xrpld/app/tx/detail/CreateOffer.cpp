...
#include <xrpl/protocol/st.h>
    ...
    namespace ripple
{
    TxConsequences CreateOffer::makeTxConsequences(PreflightContext const& ctx)
    {
    ...
    NotTEC CreateOffer::preflight(PreflightContext const& ctx)
    {
<<<<<<< HEAD
...
>>>>>>> develop
    ...
    TER CreateOffer::preclaim(PreclaimContext const& ctx)
    {
        ...
        std::uint32_t const uAccountSequence =
            sleCreator->getFieldU32(sfSequence);
        ...
        // This can probably be simplified to make sure that you cancel
        // sequences before the transaction sequence number.
        ...
        // if domain is specified, make sure that domain exists and the offer
        // create is part of the domain
        if (ctx.tx.isFieldPresent(sfDomainID))
        {
        ...
        if (asset.holds<Issue>())
        {
            ...
            if ((*issuerAccount)[sfFlags] & lsfRequireAuth)
            {
                auto const trustLine = view.read(
                    keylet::line(id, asset.getIssuer(), issue.currency));
                ...
                if (!is_authorized)
                {
                    JLOG(j.debug()) << "delay: can't receive IOUs from issuer "
                                       "without auth.";
...
=======
    ...
    bool CreateOffer::dry_offer(ApplyView & view, Offer const& offer)
    {
    ...
    bool CreateOffer::reachedOfferCrossingLimit(Taker const& taker) const
    {
    ...
    std::pair<TER, Amounts> CreateOffer::bridged_cross(
        Taker & taker,
        ApplyView & view,
        ...
        NetClock::time_point const when)
    {
        ...
        while (have_direct || have_bridge)
        {
            ...
            if (use_direct)
            {
                ...
                JLOG(j_.debug())
                    << "Direct Result: " << transToken(cross_result);
            ...
            else
            {
                ...
                cross_result =
                    taker.cross(offers_leg1.tip(), offers_leg2.tip());

                JLOG(j_.debug())
                    << "Bridge Result: " << transToken(cross_result);
                ...
                {
                    // This old behavior may leave an empty offer in the book
                    // for the second leg.
    ...
    std::pair<TER, Amounts> CreateOffer::direct_cross(
        Taker & taker,
        ApplyView & view,
        ApplyView & view_cancel,
        NetClock::time_point const when)
    {
        ...
        while (have_offer)
        {
            ...
            // We are done with crossing as soon as we cross the quality
            // boundary
            ...
            // Postcondition: If we aren't done, then we *must* have consumed
            // the
    ...
    std::pair<TER, Amounts> CreateOffer::takerCross(
        Sandbox & sb, Sandbox & sbCancel, Amounts const& takerAmount)
    {
...
>>>>>>> develop
    ...
    std::pair<TER, Amounts> CreateOffer::flowCross(
        PaymentSandbox & psb,
            ...
            if (isTesSuccess(result.result()))
            {
                ...
                else
                {
                    ...
                    if (txFlags & tfSell)
                    {
                        ...
                        // much we sold during crossing.  This preserves the
                        // offer Quality,

                        // Reduce the offer that is placed by the crossed
                        // amount. Note that we must ignore the portion of the
                        ...
                        // It's possible that the divRound will cause our
                        // subtract to go slightly negative.  So limit
                        // afterCross.in to zero.
                        if (afterCross.in < beast::zero)
                            // We should verify that the difference *is* small,
                            // but what is a good threshold to check?
                            afterCross.in.clear();
    ...
    std::pair<TER, Amounts> CreateOffer::cross(
        Sandbox & sb,
        Sandbox & sbCancel,
        Amounts const& takerAmount,
        std::optional<uint256> const& domainID)
...
=======
        ...
        std::function<void(SLE::ref, std::optional<uint256>)> const& setDir)
    {
        ...
        auto dir = keylet::quality(
            keylet::book(book), getRate(saTakerGets, saTakerPays));
        bool const bookExists = sb.exists(dir);

        auto const bookNode = sb.dirAppend(dir, offerKey, [&](SLE::ref sle) {
            // don't set domainID on the directory object since this directory
            // is for open book
...
>>>>>>> develop
    std::pair<TER, bool> CreateOffer::applyGuts(
        Sandbox & sb, Sandbox & sbCancel)
    {
        using beast::zero;
        ...
        if (result == tesSUCCESS)
        {
            ...
            if (uTickSize < Quality::maxTickSize)
            {
                ...
                else
                {
                    ...
                    saTakerGets =
                        divide(saTakerPays, rate, saTakerGets.asset());
                }
            ...
            JLOG(j_.debug())
                << "Attempting cross: " << to_string(takerAmount.in.asset())
        ...
        {
            XRPAmount reserve = sb.fees().accountReserve(
                sleCreator->getFieldU32(sfOwnerCount) + 1);

            if (mPriorBalance < reserve)
            {
                // If we are here, the signing account had an insufficient
                // reserve *prior* to our processing. If something actually
                // crossed, then we allow this; otherwise, we just claim a fee.
                if (!crossed)
...
>>>>>>> develop
    ...
    TER CreateOffer::doApply()
    {
        // This is the ledger view that we work against. Transactions are
        // applied as we go on processing transactions.
        Sandbox sb(&ctx_.view());

        // This is a ledger with just the fees paid and any unfunded or expired
        // offers we encounter removed. It's used when handling Fill-or-Kill
        // offers, if the order isn't going to be placed, to avoid wasting the
        // work we did.
