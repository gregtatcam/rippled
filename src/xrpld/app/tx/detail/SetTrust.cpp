...
SetTrust::preflight(PreflightContext const& ctx)
{
    ...
    if (badCurrency() == saLimitAmount.get<Issue>().getCurrency())
    {
...
SetTrust::preclaim(PreclaimContext const& ctx)
{
    ...
    auto const currency = saLimitAmount.get<Issue>().getCurrency();
            ...
            !ctx.view.read(keylet::line(id, uDstAccountID, currency)))
        {
                    ...
                    ctx.view.read({ltAMM, sleDst->getFieldH256(sfAMMID)}))
            {
                if (auto const lpTokens =
                    ...
                    lpTokens.get<Issue>().getCurrency() !=
                    saLimitAmount.get<Issue>().getCurrency())
                    return tecNO_PERMISSION;
            }
            else
                return tecINTERNAL;
...
SetTrust::doApply()
{
    ...
    Currency const currency(saLimitAmount.get<Issue>().getCurrency());
    ...
    saLimitAllow.get<Issue>().setIssuer(account_);
