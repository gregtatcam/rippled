...
doAMMInfo(RPC::JsonContext& context)
{
    ...
    if (!isXRP(asset1Balance))
        ammResult[jss::asset_frozen] = isFrozen(
            *ledger, ammAccountID, issue1.getCurrency(), issue1.account());
    if (!isXRP(asset2Balance))
        ammResult[jss::asset2_frozen] = isFrozen(
            *ledger, ammAccountID, issue2.getCurrency(), issue2.account());
