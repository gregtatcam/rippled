...
doGetAggregatePrice(RPC::JsonContext& context)
{
    ...
    for (auto const& oracle : params[jss::oracles])
    {
        ...
        {
            if (symbol != sle->getFieldCurrency(sfSymbol).getText() ||
                priceUnit != sle->getFieldCurrency(sfPriceUnit).getText())
            {
                RPC::inject_error(rpcOBJECT_NOT_FOUND, result);
                return result;
            }
            auto const price = sle->getFieldU64(sfSymbolPrice);
            auto const scale = -static_cast<int>(sle->getFieldU8(sfScale));
