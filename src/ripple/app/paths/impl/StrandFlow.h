    ...
    path::detail::FlowDebugInfo* flowDebugInfo = nullptr)
{
    ...
    auto deltaIn = [&]() -> std::optional<TInAmt> {
        if (!remainingIn)
            return remainingIn;
        auto const delta = Number(*remainingIn) / numPEIters;
