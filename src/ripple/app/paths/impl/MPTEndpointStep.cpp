...
//------------------------------------------------------------------------------
            ...
            std::shared_ptr<const SLE> const& sleSrc) const
    {
        ...
        // TODO MPT
        if (!ctx.view.exists(keylet::mptIssuance(mptID_)))
...
//------------------------------------------------------------------------------
    ...
    MPTEndpointStep<TDerived>::maxPaymentFlow(ReadView const& sb) const
    {
        ...
        if (src_ != issuer_)
        {
            auto const srcOwed = toAmount<MPTAmount>(
                    accountHolds(sb, src_, mptID_, issuer_, fhIGNORE_FREEZE, j_));

            return {srcOwed, DebtDirection::redeems};
        }

        if (auto const sle = sb.read(keylet::mptIssuance(mptID_)))
        {
    ...
    template <class TDerived>
    DebtDirection
    MPTEndpointStep<TDerived>::debtDirection(ReadView const& sb, StrandDirection dir)
    const
    {
            ...
            MPTAmount const& out)
    {
        ...
        Issue const srcToDstIss(mptID_);
        ...
        if (srcToDst <= maxSrcToDst)
        {
            ...
            auto const ter = rippleMPTCredit(sb, src_, dst_, toSTAmount(srcToDst, srcToDstIss), j_);
            (void)ter;
        ...
        auto const ter = rippleMPTCredit(sb, src_, dst_, toSTAmount(maxSrcToDst, srcToDstIss), j_);
        (void)ter;
...
// to prevent this.
            ...
            DebtDirection srcDebtDir)
    {
        if (cache_->in < fwdIn)
        {
            ...
            {
                if (!cache_->in.mpt() ||
                    (double(fwdIn.mpt()) / double(cache_->in.mpt())) > 1.01)
                {
...
// Returns srcQOut, dstQIn
    ...
    MPTEndpointStep<TDerived>::check(StrandContext const& ctx) const
    {
        ...
        {
            auto const ter = checkFreeze(ctx.view, src_, dst_, mptID_);
            if (ter != tesSUCCESS)
                return ter;
        }
            ...
            MPT const& a)
    {
        ...
        if (ctx.offerCrossing)
        {
            auto offerCrossingStep =
                    std::make_unique<DirectMPTOfferCrossingStep>(ctx, src, dst, a);
        ...
        {
            auto paymentStep =
                    std::make_unique<DirectMPTPaymentStep>(ctx, src, dst, a);
