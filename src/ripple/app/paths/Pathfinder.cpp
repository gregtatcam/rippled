...
{
    if (pathAsset.isCurrency())
    {
        auto const account = srcIssuer.value_or(
            isXRP(pathAsset.currency()) ? xrpAccount() : srcAccount);
        return STAmount(Issue{pathAsset.currency(), account}, 1u, 0, true);
    }
    return STAmount(pathAsset.mpt(), 1u, 0, true);
}
...
{
    if (pathAsset.isCurrency())
        return Issue{pathAsset.currency(), account};
    return pathAsset.mpt();
}
...
{
    assert(!uSrcIssuer || isXRP(uSrcPathAsset) == isXRP(uSrcIssuer.value()));
}
...
Pathfinder::issueMatchesOrigin(Asset const& asset)
{
    ...
    bool matchingAccount = isXRP(asset) ||
        (mSrcIssuer && asset.account() == mSrcIssuer) ||
        asset.account() == mSrcAccount;
    ...
    std::function<bool(void)> const& continueCallback)
{
    ...
    bool const bAuthRequired = [&]() {
        ...
        return requireAuth(*mLedger, asset.mptIssue(), account) != tesSUCCESS;
    }();
    bool const bFrozen = [&]() {
        if (pathAsset.isCurrency())
            return (aFlags & lsfGlobalFreeze) != 0;
        return isGlobalFrozen(*mLedger, asset.mptIssue());
    }();
    ...
    if (!bFrozen)
    {
        ...
        else if (auto const mpts = mAssetCache->getMPTs(account))
        {
            for (auto const& mpt : *mpts)
            {
                ...
                else if (isDstAsset && dstAccount == mpt.second)
                {
                    count += 10000;
                }
                else
            ...
            if (sleEnd)
            {
                ...
                auto forAssets = [&]<typename AssetType>(
                    ...
                    static bool constexpr isMPT =
                        std::is_same_v<AssetType, std::vector<MPT>>;
                    ...
                    for (auto const& asset : assets)
                    {
                        ...
                        ();
                        auto const direction = [&]() constexpr->std::optional<
                            LineDirection>
                        {
                            if constexpr (isLine)
                        ...
                        auto const correctAsset = [&]() {
                            if constexpr (isLine)
                                return uEndPathAsset.currency() ==
                                    asset.getLimit().getCurrency();
                            if constexpr (isMPT)
                                return uEndPathAsset.mpt() == asset;
                        }();
                        auto checkLine = [&]() {
                            if constexpr (isLine)
                            {
        ...
        else
        {
            ...
            for (auto const& book : books)
            {
                ...
                if (!currentPath.hasSeen(
                        xrpAccount(), book.out, book.out.account()) &&
                    !issueMatchesOrigin(book.out) &&
                    (!bDestOnly || equalAssets(book.out, mDstAmount.asset())))
                {
                    STPath newPath(currentPath);

                    if (book.out.isIssue() && isXRP(book.out.issue().currency))
                    {  // to XRP
                        ...
                        if (isXRP(mDstAmount.asset()))
                        {
                            ...
                            incompletePaths.push_back(newPath);
                                 ...
                                 book.out.account()))
                    {
                        auto const assetType = book.out.isIssue()
                            ? STPathElement::typeCurrency
                        ...
                        if (hasEffectiveDestination &&
                            ...
                            equalAssets(book.out, mDstAmount.asset()))
                        {
