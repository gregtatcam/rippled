...
ValidCFTIssuance::visitEntry(
    ...
    std::shared_ptr<SLE const> const& after)
{
    if (!before && after && after->getType() == ltCFTOKEN_ISSUANCE)
    {
        cftsCreated_++;
    }
    if (before && before->getType() == ltCFTOKEN_ISSUANCE && after &&
        after->getType() == ltCFTOKEN_ISSUANCE)
    {
        if (isDelete)
            cftsDeleted_++;
    }
    ...
    auto check = [this, isDelete](std::shared_ptr<SLE const> const& sle) {
...

        {
            ...
            for (auto const& obj : cftokens)
            {
                ...
                // TODO - should move compareTokens to a shared namespace. we do
                // use the same mask - and confirm this is even correct
                if (!nft::compareTokens(loCmp, tokenID))
                    badSort_ = true;
                loCmp = tokenID;
