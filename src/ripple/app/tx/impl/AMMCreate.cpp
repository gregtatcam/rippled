...
AMMCreate::preflight(PreflightContext const& ctx)
{
    ...
    {
        JLOG(ctx.j.debug())
            << "AMM Instance: tokens can not have the same Issue/MPT.";
...
AMMCreate::preclaim(PreclaimContext const& ctx)
{
    ...
    auto noDefaultRipple = [](ReadView const& view, Asset const& asset) {
        if (asset.isMPT() || isXRP(asset))
            return false;

        if (auto const issuerAccount =
                view.read(keylet::account(asset.account())))
    ...
    auto insufficientBalance = [&](STAmount const& amount) {
        ...
        return accountID != amount.asset().account() &&
            accountHolds(
                ctx.view,
                accountID,
                amount.asset(),
    ...
    auto isLPToken = [&](STAmount const& amount) -> bool {
        if (auto const sle =
                ctx.view.read(keylet::account(amount.asset().account())))
    ...
    auto clawbackDisabled = [&](Asset const& asset) -> TER {
        ...
        {
            if (auto const sle =
                    ctx.view.read(keylet::mptIssuance(asset.mptIssue().mpt()));
                !sle)
                return tecINTERNAL;
        ...
        {
            if (auto const sle =
                    ctx.view.read(keylet::account(asset.account()));
                !sle)
                return tecINTERNAL;
    ...
    auto checkMPT = [&](Asset const& asset) {
        if (asset.isMPT())
            return ctx.view.read(keylet::mptIssuance(asset.mptIssue().mpt())) !=
                nullptr;
        return true;
    };
    ...
    auto sendAndInitTrustOrMPT = [&](STAmount const& amount) -> TER {
        ...
        if (amount.isMPT())
        {
            auto const mptokenKey =
                keylet::mptoken(amount.mptIssue().mpt(), *ammAccount);
            ...
            (*mptoken)[sfMPTokenIssuanceID] = amount.mptIssue().getMptID();
            (*mptoken)[sfFlags] = 0;
        ...
        if (amount.isIssue() && !isXRP(amount))
        {
