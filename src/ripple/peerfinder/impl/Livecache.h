...
{
private:
    using cache_type = beast::aged_unordered_set<
        beast::IP::Endpoint,
...
template <class Allocator>
void
Livecache<Allocator>::insert(beast::IP::Endpoint const& ep)
{
    auto result = m_cache.emplace(ep);
    if (result.second)
    {
