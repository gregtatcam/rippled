...
class Asset
{
    inline static uint256 none = uint256{0};
...
public:
    ...
    {
        assert(
            !(std::holds_alternative<uint256>(asset_) &&
              std::get<uint256>(asset_) == none));
        if (std::holds_alternative<uint256>(asset_) &&
            std::get<uint256>(asset_) == none)
            Throw<std::logic_error>("Uninitialized asset");
    }
