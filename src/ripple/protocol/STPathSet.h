    ...
    : mType(typeNone)
{
    ...
    {
        mAssetID = *asset;
        mType |= mAssetID.isCurrency() ? typeCurrency : typeMPT;
    }
    ...
    , is_offer_(isXRP(mAccountID))
{
    ...
    if (!asset.isMPT() && (forceCurrency || !isXRP(mAssetID.currency())))
        mType |= typeCurrency;
    ...
    , is_offer_(isXRP(mAccountID))
{
    ...
    else if (mAssetID.isCurrency() && isXRP(mAssetID.currency()))
...
{
    return mAssetID.currency();
}

inline MPT const&
STPathElement::getMPT() const
{
    return mAssetID.mpt();
}
