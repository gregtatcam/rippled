...
STPathElement::get_hash(STPathElement const& element)
{
    ...
    if (element.getPathAsset().isMPT())
    {
        hash_currency += beast::uhash<>{}(element.getPathAsset().mpt());
    }
    else
    {
        for (auto const x : element.getPathAsset().currency())
...
Json::Value STPath::getJson(JsonOptions) const
{
    ...
    for (auto it : mPath)
    {
        ...
        if (iType & STPathElement::typeMPT)
            elem[jss::mpt_issuance_id] = to_string(getMptID(it.getMPT()));
...
STPathSet::add(Serializer& s) const
{
    ...
    for (auto const& spPath : value)
    {
        ...
        for (auto const& speElement : spPath)
        {
            ...
            if (iType & STPathElement::typeMPT)
                s.addBitString(getMptID(speElement.getMPT()));
