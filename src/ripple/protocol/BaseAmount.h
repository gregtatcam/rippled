...
public:
    ...
    explicit BaseAmount(std::uint64_t mantissa = 0, bool negative = false);

    explicit BaseAmount(BaseAmount const& amt);
    ...
    BaseAmount&
    operator+=(BaseAmount const&);
    BaseAmount&
    operator-=(BaseAmount const&);

    BaseAmount& operator=(beast::Zero);
...
protected:
    ...
    friend BaseAmount
    operator+(BaseAmount const& v1, BaseAmount const& v2);
};
...
template <ValidAsset T>
std::uint64_t const BaseAmount<T>::uRateOne =
    getRate(BaseAmount<T>(1), BaseAmount<T>(1));

template <ValidAsset T>
void
BaseAmount<T>::setAsset(T const& asset)
{
    mAsset = asset;
}
...
template <ValidAsset T>
BaseAmount<T>
divide(BaseAmount<T> const& num, BaseAmount<T> const& den, T const& asset)
{
    if (den == beast::zero)
        Throw<std::runtime_error>("division by zero");
...
template <ValidAsset T>
BaseAmount<T>
multiply(BaseAmount<T> const& v1, BaseAmount<T> const& v2, T const& asset)
{
    if (v1 == beast::zero || v2 == beast::zero)
        return BaseAmount<T>(asset);
...
template <ValidAsset T>
std::uint64_t
getRate(BaseAmount<T> const& offerOut, BaseAmount<T> const& offerIn)
{
