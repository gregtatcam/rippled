...
isGlobalFrozen(ReadView const& view, Issue const& issue)
{
    ...
    if (issue.isMPT())
    {
        if (auto const sle = view.read(keylet::mptIssuance(issue.getMPT())))
    ...
    Issue const& issue)
{
    ...
    if (issue.account() != account)
    {
        ...
        {
            if (auto const sle =
                    view.read(keylet::mptoken(issue.getMPT(), account)))
                return sle->getFlags() & lsfMPTLocked;
        }
        // Check if the issuer froze the line
        else if (
            auto const sle = view.read(
                keylet::line(account, issue.account(), issue.getCurrency())))
        {
            return sle->isFlag(
...
{
    Issue const issue = asset.isMPT() ? Issue{asset.getMPT()}
                                      : Issue{asset.getCurrency(), issuer};
    if (isXRP(issue))
        return false;
    ...
    beast::Journal j)
{
    ...
    auto getSle = [&]() -> std::shared_ptr<SLE const> {
        Keylet const key = asset.isMPT()
            ? keylet::mptoken(asset.getMPT(), account)
            : keylet::line(account, issuer, asset.getCurrency());
    ...
    if (auto const sle = getSle())
    {
        ...
        if (asset.isMPT())
        {
            ...
            if (amt > locked)
                amount = STAmount{Issue{asset.getMPT()}, amt - locked};
        }
        else
        {
    ...
    beast::Journal j)
{
    ...
    auto const index =
        keylet::line(issue.account(), account, issue.getCurrency());
    ...
    STAmount const limit({issue.getCurrency(), account});
...
requireAuth(ReadView const& view, Issue const& issue, AccountID const& account)
{
    ...
    if (issue.isMPT())
    {
        auto const mptID = keylet::mptIssuance(issue.getMPT());
