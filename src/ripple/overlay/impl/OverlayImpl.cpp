    ...
    std::unique_ptr<stream_type>&& stream_ptr)
{
    ...
    // Note that the peer is activated now. This is consistent with
    // the previous code where activation was done separately but before
    // the response was sent and possibly in a nested call (i.e. sync)
    // if calling on the strand's thread. Both in/out bound will be
    // consistently handled once Start Protocol PR is merged.
    add_active(peer);
