...
void
P2POverlayImpl::add_active(std::shared_ptr<PeerImp<P2PeerImp>> const& peer)
{
    std::lock_guard lock(mutex_);
...
void
P2POverlayImpl::remove(std::shared_ptr<PeerFinder::Slot> const& slot)
{
    std::lock_guard lock(mutex_);
...
*/
void
P2POverlayImpl::activate(std::shared_ptr<PeerImp<P2PeerImp>> const& peer)
{
    // Now track this peer
    {
        std::lock_guard lock(mutex_);
...
void
P2POverlayImpl::onPeerDeactivate(Peer::id_t id)
{
    std::lock_guard lock(mutex_);
    ids_.erase(id);
}
...
P2POverlayImpl::findPeerByShortID(Peer::id_t const& id) const
{
    std::lock_guard lock(mutex_);
...
std::shared_ptr<P2Peer>
P2POverlayImpl::findPeerByPublicKey(PublicKey const& pubKey)
{
    std::lock_guard lock(mutex_);
